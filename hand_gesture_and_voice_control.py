...
# Function to perform system commands based on transcription
def perform_command(transcription):
    ...
    # === Google Search Bar Commands ===
        ...
        if text_to_type:
            pyautogui.hotkey('ctrl', 'l')  # Focus on the browser's address bar
            time.sleep(0.5)
            pyautogui.typewrite(text_to_type)
        else:
            print("No text provided after the command.")
    elif any(keyword in transcription for keyword in ["show", "enter", "press"]):
...
def hand_gesture_control():
    """Handle hand gesture detection and actions."""
    global plocX, plocY, clocX, clocY, selection_mode
    ...
    while True:
        ...
        if lmList:
            ...
            if fingers[1] == 1 and fingers[2] == 0 and not selection_mode:
...
# Function to send and receive data from WebSocket
async def send_receive():
    print(f'Connecting websocket to URL {URL}')

    # Connect with headers for authorization
        ...
        async def receive():
            ...
            while True:
                try:
                    ...
                    current_text = json.loads(result_str).get('text', '')

                    # If there's a new part of transcription, add it to accumulated text
                    if current_text:
                        accumulated_text = current_text
                        last_received_time = time.time()  # Reset the timer on new text

                    # Check if there's a pause in receiving new text
                    if time.time() - last_received_time > pause_threshold and accumulated_text:
                        # Print and process the accumulated text after the pause
                        print("Transcription:", accumulated_text)
                        perform_command(accumulated_text)  # Call perform_command function
                        accumulated_text = ""  # Clear after processing
...
# Run hand gesture control and AssemblyAI voice command concurrently
if __name__ == "__main__":
    hand_thread = threading.Thread(target=hand_gesture_control)
    voice_thread = threading.Thread(target=lambda: asyncio.run(send_receive()))
